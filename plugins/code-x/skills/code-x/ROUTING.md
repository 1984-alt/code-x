...
**AUDIT STAGE (A-PROP-001 + PBAF-PROP-001, v1.22)** — the auditor is always the **opposite model family from the builder** (existing Code-X norm), TOP tier (judgment call, not mechanical). The review ladder runs in order — CodeRabbit → same-family self-review → cross-family — and the final audit receipt must be authored by the family opposite the builder; a cross-family receipt not preceded, in order, by CodeRabbit + self-review is invalid (`AUDIT-STAGE-REVIEW-LADDER`). `AUDIT_STAGE` is a first-class `current_stage` value (mirrors `FIXING_STAGE`, F-PROP-001) with its own `audit_stage` orchestrator seat in `BUILD-ENGINE-PROFILES.yaml` (verify/review-tier, not a builder seat; `AUDIT-STAGE-SEAT-PROFILE` bites a session with no seat). **Route guard (X4):** the final xfam/ship route requires a valid, PASSing v1.22 `audit_stage_final` receipt (`cx check audit --final`, judged by `collect_audit_findings`) — the legacy `built_app_audit` field is additive, never a substitute; `cx check final-ready` blocks on either being missing (`AUDIT-STAGE-FINAL-READY-CHAIN`). PROP prefix: `A-PROP` (Audit-only) or `PBAF-PROP` (touches all four stages) — naming scheme extended to sorted canonical order **P-B-A-F** (CEO-D ruling 2, 2026-07-01).
