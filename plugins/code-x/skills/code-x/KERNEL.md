...
- **SEE-AND-TEST: demo every user-facing module on its real screen before accepting it (PBF-PROP-012 Part E).** After the verify-app agent passes, and BEFORE the CEO live-drive accept, the orchestrator must open the running build on the correct surface — **web → Chrome** (record `live_url`) · **mobile → iPhone 13 Pro simulator** (record `simulator: iphone-13-pro`) — capture a real screenshot (hash-bound into the receipt), and obtain the CEO's typed accept token that embeds the build's `repo_sha` prefix. The `module_demo` block is a **precondition** inside `validate_live_slice_accept`: no valid demo block → the module is not accepted and the next module is blocked. This directly fixes the sample root cause where `ceo_drove: true` was self-ticked without a real show-step. **Residual honest limit:** a checker cannot prove the CEO's eyes were on the screen or that the token was typed by the CEO — that "human saw it and drove it" stays human-attested; E does not claim otherwise. Declare the demo commitment once in `session_start.module_demo_mode` (`demo_every_user_facing_module: yes`); a backend-only project may carry a typed `no_user_facing_modules` waiver + `ceo_decision_ref`. [RULE:module-demo-mode-ack]
