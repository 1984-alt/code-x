# cmd_drift: the DRIFT ALARM (BF-PROP-007 Lever C, fold v1.15).
#
...
#     NEVER a blocking exit code. (Same advisory-until-provable pattern B-PROP-009 used for golden-drift.)
#
...
def _scope_overreach(card: dict, cards_by_id: dict) -> list:
    ...
    if outside:
        findings.append(("P1",
            ...
            "the anchored card was allowed to touch (BF-PROP-007 Lever C / LOCK-FIDELITY-RESTORE-OVERREACH)"))
    return findings
        ...
        for rid in sorted(card_requirement_ids(card)):
            if rid not in req_ids and not is_authorized_scope_change:
                findings.append(("P1", f"card '{cid}'",
                    ...
                    "lock (BF-PROP-007 Lever C / LOCK-FIDELITY-DRIFT-UNLOGGED)"))
    ...
    for rid, disp in req_ids.items():
        if disp == "BUILDING" and rid not in all_covered:
            findings.append(("P1", "frozen manifest",
                ...
                "build progresses) (BF-PROP-007 Lever C / LOCK-FIDELITY-DRIFT-UNLOGGED)"))
    ...
    for cid in working_set:
        ...
        if str(card.get("mode", "")) == "FIX" and str(card.get("deviation_class", "")) == "AMBIGUITY_RESOLVED":
            advisories.append(
                ...
                "'behavior exceeds requirement' is not mechanically decidable) (BF-PROP-007 Lever C)")
...
def open_lock_deviation_blockers(state: dict, loc: str, gate: str) -> list:
    """Shared helper (BF-PROP-007): an OPEN lock_deviation row blocks the named accept gate.
    ...
    if not isinstance(rows, list):
        findings.append(("P1", loc,
            "lock_deviations must be a list of typed rows (BF-PROP-007)"))
        return findings
    from cx_lock_fidelity import VALID_DEVIATION_CLASSES, path_is_unsafe
    for i, row in enumerate(rows):
        if not isinstance(row, dict):
            findings.append(("P1", loc, f"lock_deviations[{i}] is not a mapping (BF-PROP-007)"))
            continue
        ...
        if dc == "SCOPE_CHANGE":
            for refk in ("ceo_decision_ref", "packet_amendment_ref"):
                if not row.get(refk):
                    findings.append(("P1", loc,
                        f"lock_deviations[{i}] is SCOPE_CHANGE without {refk} (BF-PROP-007)"))
        for refk in ("ceo_decision_ref", "packet_amendment_ref"):
            v = str(row.get(refk, "") or "").strip()
            if v and path_is_unsafe(v) and "/" in v:
                findings.append(("P1", loc,
                    f"lock_deviations[{i}].{refk} '{v}' is an unsafe path (absolute / '..') (BF-PROP-007)"))
        ...
        # whose status the wall does not understand must NOT slip through as if reviewed (BF-PROP-007 xfam F8).
        ...
        if status == "OPEN":
            findings.append(("P1", loc,
                ...
                "logged ambiguity can never quietly ship (BF-PROP-007)"))
            ...
            findings.append(("P1", loc,
                ...
                "fail closed, never slip through as reviewed (BF-PROP-007)"))
    return findings
