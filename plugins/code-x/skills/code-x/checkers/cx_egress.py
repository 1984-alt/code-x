# cmd_egress: the scrub-before-egress gate for a mandatory external reviewer (PROP-026).
#
...
# CANNOT be left to memory (GPT #1, the P0 of the PROP-026 review). Before a diff is sent
...
#       PROP-022 discipline); a scrub receipt contradicted by remaining sensitive content
...
def cmd_egress(args) -> int:
    ...
    if isinstance(scrub, dict):
        for k in SCRUB_REQUIRED_KEYS:
            if not field_present(scrub, k):
                findings.append(("P0", loc,
                    ...
                    "it scrubbed, the scrub command, a positive control, and produced_at (PROP-026)"))
        if findings:
            return findings_report(findings)
        if str(scrub.get("target")) != target:
            findings.append(("P0", loc,
                f"egress_scrub.target '{scrub.get('target')}' != --target '{target}' (PROP-026)"))
        if str(scrub.get("diff_hash")) != diff_hash:
            findings.append(("P0", loc,
                f"egress_scrub.diff_hash {scrub.get('diff_hash')} != the diff's sha12 {diff_hash} — the "
                "scrub receipt is not bound to THIS diff (stale / forged, or the diff changed) (PROP-026)"))
        try:
        ...
        if pce == 0:
            findings.append(("P0", loc,
                ...
                "planted secret) is not caught is not proven to work (PROP-022 positive-control discipline)"))
        if hits and not findings:
            findings.append(("P0", loc,
                f"egress_scrub claims a clean diff but sensitive content remains in it ({hits}) — the scrub "
                "receipt is contradicted by the diff content (PROP-026 / GPT #1; a PASS contradicted by "
                "evidence is a P0)"))
        if not findings:
    ...
    findings.append(("P0", loc,
        ...
        f"is uploaded to an external reviewer{detail} (PROP-026 / GPT #1)"))
    return findings_report(findings)
